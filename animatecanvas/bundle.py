"""AnimateCanvas checkpoint loading and inference operations.

This bundle holds a HunyuanMotionMMDiT transformer and provides atomic
forward functions used by the standalone inference pipeline:
...
class MotionCanvasBundle(nn.Module):
    """Inference bundle with checkpoint-compatible module and buffer names."""
    ...
    def __init__(
        ...
        from animatecanvas.network import MotionCanvasMMDiT
        network_cfg = deepcopy(motion_transformer)
        network_type = network_cfg.pop('type', 'MotionCanvasMMDiT')
        if network_type not in ('MotionCanvasMMDiT', 'HunyuanMotionMMDiT'):
            raise ValueError(f'Unsupported network: {network_type}')
        if network_cfg.get('trainable') == 'lora':
            raise ValueError('This inference package requires a full checkpoint, not a LoRA adapter')
        for key in ('trainable', 'save_ckpt', 'checkpoint_format', 'gradient_checkpointing'):
            network_cfg.pop(key, None)
        self.motion_transformer = MotionCanvasMMDiT(**network_cfg)
        ...
        if t2m_pretrained_path:
            raise ValueError('Use a full exported AnimateCanvas checkpoint, not training initialization')
    ...
    @property
    def body_model(self):
        """Mesh body assets are not needed for skeleton-only inference."""
        return None
    ...
    def encode_text(self, text: List[str]) -> Dict[str, Tensor]:
        ...
        if not hasattr(self, '_text_encoder') or self._text_encoder is None:
            ...
            from animatecanvas.network.text_encoder import (
                HYTextModel,
            )
    ...
    def decode_motion_from_latent(
        ...
        """
        from animatecanvas.network.geometry import rot6d_to_rotation_matrix
        ...
        if self.rotation_space == 'global':
            from animatecanvas.kinematics.fk import global_to_local_rot6d
        ...
        from animatecanvas.kinematics.fk import differentiable_fk
        k3d, _ = differentiable_fk(rot6d_to_rotation_matrix(rot6d), transl, self.get_bone_offsets())
    ...
    def get_bone_offsets(self) -> Tensor:
        """Return the checkpoint's authoritative (22, 3) rest-skeleton offsets."""
        ...
        raise RuntimeError('The checkpoint must include bone_offsets_22.pt for skeleton decoding')
